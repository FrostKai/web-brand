...
<div class="admin-wrapper">
  ...
  <div class="admin-content">
    <div class="content-header">
      <div>
        <h1 class="content-title">Edit Product: {{ $product->name }}</h1>
        <p style="color: var(--text-muted); font-size: 0.95rem; margin-top: 4px;">Update information and specifications of this sound device.</p>
      </div>
    </div>
      ...
      <form method="POST" action="{{ route('admin.products.update', $product->id) }}">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="text" name="name" class="form-input" value="{{ old('name', $product->name) }}" required placeholder="e.g., AeroBuds Elite" />
          </div>
          <div class="form-group">
            <label class="form-label">Category</label>
            <select name="category" class="form-input" required>
              <option value="headphones" {{ old('category', $product->category) === 'headphones' ? 'selected' : '' }}>Headphones</option>
              <option value="earbuds" {{ old('category', $product->category) === 'earbuds' ? 'selected' : '' }}>Earbuds</option>
              <option value="speakers" {{ old('category', $product->category) === 'speakers' ? 'selected' : '' }}>Speakers</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" name="price" step="0.01" class="form-input" value="{{ old('price', $product->price) }}" required placeholder="149.99" />
          </div>
          <div class="form-group">
            <label class="form-label">Old Price ($) <span style="font-weight: normal; color: var(--text-muted); font-size: 0.8rem;">(Optional)</span></label>
            <input type="number" name="old_price" step="0.01" class="form-input" value="{{ old('old_price', $product->old_price) }}" placeholder="199.99" />
          </div>
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="text" name="badge" class="form-input" value="{{ old('badge', $product->badge) }}" placeholder="e.g., New, Sale, Best Seller" />
          </div>
          <div class="form-group">
            <label class="form-label">Badge Style Type</label>
            <select name="badge_type" class="form-input">
              <option value="">None</option>
              <option value="new" {{ old('badge_type', $product->badge_type) === 'new' ? 'selected' : '' }}>New (Blue)</option>
              <option value="sale" {{ old('badge_type', $product->badge_type) === 'sale' ? 'selected' : '' }}>Sale (Red)</option>
              <option value="popular" {{ old('badge_type', $product->badge_type) === 'popular' ? 'selected' : '' }}>Popular (Gold)</option>
            </select>
          </div>
        </div>
        ...
        <div class="form-group">
          ...
          <input type="url" name="image_url" class="form-input" value="{{ old('image_url', $product->image_url) }}" placeholder="https://images.unsplash.com/... or /images/..." />
          <p style="font-size: 0.78rem; color: var(--text-muted); margin-top: 6px;">Provide a clean image URL to render the product card properly.</p>
        </div>

        <div class="form-group">
          <label class="form-label">Product Description</label>
          <textarea name="description" class="form-textarea" required placeholder="Describe the sonic attributes, battery life, design comfort...">{{ old('description', $product->description) }}</textarea>
        </div>
